function f1 = convplots_fig%(export_txt,k)
...
for i=1:length(n_vect)
    ...
    for j=1:length(tau_vect)
        tau=tau_vect(j);
                err=load(['Errors/errors_SAV_n',num2str(meshno),'_tau',num2str(tau),'.txt']);
                % L^\infty errors on [0,T]
                error_phi(i,j)=max(err(2,:));
                error_mu(i,j)=max(err(3,:));
                error_r(i,j)= max(err(4,:));
...
% 0 0
subplot(1,3,1)
loglog_conv(tau_vect,error_phi)
hold on;
plot(tau_vect,tau_vect,'-- black','LineWidth',1);
hold off; 
h1=title('$\|\phi-(\phi^n)\|_{L^\infty(L^2)}$');
...
% 0 0
subplot(1,3,2)
loglog_conv(tau_vect,error_mu)
hold on;
plot(tau_vect,tau_vect,'-- black','LineWidth',1);
hold off; 
h1=title('$\|\mu-(\mu^n)\|_{L^\infty(L^2)}$');
set(h1,'Interpreter','latex','FontSize',18);
h1=xlabel('step size ($\tau$)');
set(h1,'Interpreter','latex','FontSize',16);
h2=legend(DOF,'Location',location_text,'FontSize',10);
...
% % fig and eps
% saveas(f1,['Quad_Pot_time_Linfty.fig']);
